...
{
	m_impl = new CMfVideoEncoder::Impl();
}
...
{
	return m_impl->initialise(filePath, width, height, frameRate);
}
...
{
	return m_impl->commitCpuFrame(pPixels, pixelSize, isRgba);
}

#ifdef MF_GPU_TEXTURE
bool CMfVideoEncoder::commitGpuFrame(void* pD3D11Texture2D)
{
	return m_impl->commiGpuFrame(pD3D11Texture2D);
}
#endif
