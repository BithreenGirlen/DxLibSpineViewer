...
void spine_tool_dialogue::Display(SSpineToolDatum& spineToolDatum, bool* pIsOpen)
{
	...
	CSpinePlayerDynamic* pDxLibSpinePlayerDynamic = static_cast<CSpinePlayerDynamic*>(spineToolDatum.pSpinePlayer);
	ISpinePlayer* pDxLibSpinePlayer = pDxLibSpinePlayerDynamic->get();
	if (pDxLibSpinePlayer == nullptr)return;
	...
	if (ImGui::BeginTabBar("Parameter category", ImGuiTabBarFlags_None))
	{
		...
		if (ImGui::BeginTabItem("Size/Scale"))
		{
			...
			ImGui::Text("Skeleton size: (%.2f, %.2f)", baseSize.u, baseSize.v);
			ImGui::Text("Offset: (%.2f, %.2f)", offset.u, offset.v);
