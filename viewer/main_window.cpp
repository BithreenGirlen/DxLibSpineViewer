...
void CMainWindow::MenuOnOpenFolder()
{
	...
	if (!wstrPickedupFolderPath.empty())
	{
		...
		{
			ClearFolderPathList();
			win_filesystem::GetFilePathListAndIndex(wstrPickedupFolderPath, nullptr, m_folders, &m_nFolderIndex);
		}
	}
}
/*取り込みファイル設定*/
...
void CMainWindow::MenuOnImportCocos()
{
	...
	LoadSpinesFromMemory(atlasData, textureDirectories, skeletonData, wstrSelectedFilePath.c_str());
}
...
void CMainWindow::MenuOnAddFile()
{
	...
	std::wstring wstrAtlasFilePath = win_dialogue::SelectOpenFile(L"atlas file", L"*.atlas;*.atlas.txt", L"Select atlas file to add", m_hWnd, true);
	if (wstrAtlasFilePath.empty())return;

	std::wstring wstrSkeletonFilePath = win_dialogue::SelectOpenFile(L"skeleton file", L"*.skel;*.bin;*.json;*.txt", L"Select skeleton file to add", m_hWnd, true);
	if (wstrSkeletonFilePath.empty())return;

	std::string strAtlasFilePath = win_text::NarrowUtf8(wstrAtlasFilePath);
	std::string strSkeletonFilePath = win_text::NarrowUtf8(wstrSkeletonFilePath);

	/* Todo: This will load file twice. */
	std::string skeletonFileDatum = win_filesystem::LoadFileAsString(wstrSkeletonFilePath.c_str());
	...
	m_dxLibSpinePlayer.get()->addSpineFromFile(strAtlasFilePath.c_str(), strSkeletonFilePath.c_str(), isBinarySkel);
}
...
bool CMainWindow::LoadSpineFilesInFolder(const std::wstring& folderPath)
{
	...
	win_filesystem::CreateFilePathList(folderPath.c_str(), L"*", wstrFilePaths);
...
std::wstring CMainWindow::BuildExportFilePath()
{
	...
	wstrFilePath += win_text::WidenUtf8(m_dxLibSpinePlayer.get()->getCurrentAnimationName());
